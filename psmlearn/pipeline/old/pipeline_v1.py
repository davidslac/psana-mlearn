...
if __name__ == '__main__':
    pipeline = Pipeline()
    pipeline.addstep('A',a)
    pipeline.addstep('B',b)
    pipeline.addstep('B',b)
    
parserEpilog = '''
generic pipeline is:

1. data stats - the model may need some overall stats of the data, i.e, channel means
2. data layers - run the data through the model and save some output layers
3. plot final - raw plot of final layer
4. feature selection - select some subset of the output layers (eliminate dead neurons, etc)
5. view tsne of selected features
6. train classifier
7. test classifier
...
def _getDefaultPipelineParser(output_dir, 
    ...
    for name in step_names:
        ...
        parser.add_argument('--%s' % redo, action='store_true', help='do or redo %s' % name, default=False)

    return parser

class _Step(object):
    def __init__(self, info, instance, input_files=None, output_files=None):
        self.name = info['name']
        self.info = info
    ...
    def __str__(self):
        msg='step=%s' % self.name
        for flist,nm in zip([self.input_files, self.output_files],
                            ['input','output']):
            if flist:
                msg += ' %s=' % nm
                msg += ','.join([os.path.basename(fname) for fname in flist])
        return msg

    def redo(self, args):
        flag = _redo_flag_name(self.name)
        try:
            val = getattr(args, flag)
        except:
            raise Exception("_Step could not find %s in args" % flag)
        return flag
    
    def run(self, data_stats=None):
        try:
            method = getattr(self.instance, self.name)
        except:
            raise Exception("_Step: could not find method: %s in instance=%s of type=%s. Derived pipeline classes must implement methods with step names." % (self.name, self.instance, type(self.instance)))
        preprocess=self.info['data_gen_preprocess']
        data_iter = self.data_generator(randomize=self.info['data_gen_randomize'],
                                        labels=self.info['data_gen_labels'],
                                        meta=self.info['data_gen_meta'])
        method(data_iter, preprocess, data_stats, self.input_files, self.output_files)
